...
namespace CONATRADEC_API.Models
{
    public sealed class AlertasAgricolasDbContext : DbContext
    {
        public AlertasAgricolasDbContext(
            DbContextOptions<
                AlertasAgricolasDbContext> options)
            : base(options)
        {
        }

        public DbSet<ConfiguracionAlertaAgricola>
            Configuraciones =>
                Set<ConfiguracionAlertaAgricola>();

        public DbSet<SeguimientoAlertaAgricola>
            Seguimientos =>
                Set<SeguimientoAlertaAgricola>();

        public DbSet<HistorialAlertaAgricola>
            Historial =>
                Set<HistorialAlertaAgricola>();

        protected override void OnModelCreating(
            ModelBuilder modelBuilder)
        {
            modelBuilder
                .Entity<ConfiguracionAlertaAgricola>()
                .HasIndex(x => x.Clave)
                .IsUnique();

            modelBuilder
                .Entity<SeguimientoAlertaAgricola>()
                .HasIndex(x => new
                {
                    x.TerrenoId,
                    x.TipoAlerta,
            ...
            modelBuilder
                .Entity<HistorialAlertaAgricola>()
                .HasIndex(x => new
                {
                    x.SeguimientoAlertaAgricolaId,
                    x.FechaUtc
                });
        }
    }
    ...
    public sealed class SeguimientoAlertaAgricola
    {
        ...
        public DateTime? FechaCierreUtc
        {
            get;
            set;
        }
        ...
        public int UsuarioUltimaModificacionId
        {
            get;
            set;
        }
