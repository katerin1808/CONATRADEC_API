...
namespace CONATRADEC_API.Auditing
{
    public sealed class AuditSaveChangesInterceptor : SaveChangesInterceptor
    {
        ...
        public AuditSaveChangesInterceptor(AuditRequestContext auditContext)
        {
        ...
        private void PrepararCambios(DbContext? dbContext)
        {
            pendientes = new List<CambioPendiente>();

            if (!auditContext.Activo || dbContext == null)
                return;

            dbContext.ChangeTracker.DetectChanges();

            IEnumerable<EntityEntry> entradas = dbContext.ChangeTracker
                .Entries()
                .Where(x =>
                ...
                .Where(x =>
                    ...
                    x.Metadata.ClrType.Name != "BitacoraDetalle");
            ...
            foreach (EntityEntry entrada in entradas)
            {
                ...
                List<PropertyEntry> propiedades = entrada.State == EntityState.Modified
                    ? entrada.Properties.Where(x => x.IsModified).ToList()
                ...
                foreach (PropertyEntry propiedad in propiedades)
                {
                    ...
                    {
                        anteriores[nombre] = NormalizarValor(
                ...
                pendientes.Add(new CambioPendiente
                {
                    ...
                    Operacion = operacion,
                    ValoresAnteriores = Serializar(anteriores),
                    ValoresNuevos = Serializar(nuevos),
                    PropiedadesModificadas = Serializar(modificadas)
                });
            }
        }
        ...
        {
            if (!auditContext.Activo || pendientes.Count == 0)
            {
                pendientes.Clear();
                return;
            }

            foreach (CambioPendiente pendiente in pendientes)
            {
                auditContext.AgregarCambio(new AuditEntityChange
                {
                    FechaHoraUtc = DateTime.UtcNow,
                    Entidad = pendiente.Entidad,
                    EntidadId = ObtenerLlave(pendiente.Entrada),
                    Operacion = pendiente.Operacion,
                    ValoresAnteriores = pendiente.ValoresAnteriores,
                    ValoresNuevos = pendiente.ValoresNuevos,
                    PropiedadesModificadas = pendiente.PropiedadesModificadas
                });
            }
        ...
        private static string ObtenerLlave(EntityEntry entrada)
        {
            ...
            {
                object? valor = entrada.Property(propiedad.Name).CurrentValue;
        ...
        private static object? NormalizarValor(object? valor)
        {
            return valor switch
            {
                ...
                DateOnly fecha => fecha.ToString("yyyy-MM-dd"),
                TimeOnly hora => hora.ToString("HH:mm:ss.fffffff"),
                string texto => AuditSanitizer.Truncar(texto, 3000),
                _ => valor
            };
        }
        ...
        {
            public EntityEntry Entrada { get; init; } = null!;
